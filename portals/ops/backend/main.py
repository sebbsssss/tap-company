...
def _normalize_ticket(ticket: dict, comments: Optional[list] = None) -> dict:
    """Flatten nested CRM fields into the shape the frontend expects.

    CRM shape: category.service, category.name, priority.name,
    raised_by.name, room.unit.prop.name, statuses[].status, assigned_to.name
    """
    enriched = _enrich_ticket(ticket, comments)
    ...
    if not isinstance(raised_by, dict):
        raised_by = {}
    assigned_to = ticket.get("assigned_to") or {}
    if not isinstance(assigned_to, dict):
        assigned_to = {}

    # Determine current status: prefer explicit status field, then statuses list
    current_status = ticket.get("status") or ""
    statuses = ticket.get("statuses") or []
    if not current_status and statuses:
        try:
            latest_s = max(statuses, key=lambda s: s.get("created", ""))
            current_status = latest_s.get("status", "")
        except Exception:
            pass

    priority_raw = pri.get("name") if isinstance(pri, dict) else str(pri or "")
    priority_norm = priority_raw.lower() if priority_raw else ""

    last_update = (
        ticket.get("updated") or ticket.get("modified") or ticket.get("created")
    )

    return {
        **enriched,
        "tenant": raised_by.get("name") or "",
        "property": prop.get("name") or "",
        "category": cat.get("name") or "",
        "service": cat.get("service") or "",
        "area": prop.get("area") or unit.get("name") or "",
        "room": room.get("number") or "",
        "assignee": assigned_to.get("name") or "",
...
def list_tickets(
    ...
    # Post-filter (CRM doesn't support all filters natively) — run before normalization
    if property:
        tickets = [t for t in tickets if _prop_name(t) == property]
    if assignee:
        tickets = [
            t for t in tickets
    ...
    if priority:
        tickets = [
            t for t in tickets
            if ((t.get("priority") or {}).get("name", "") if isinstance(t.get("priority"), dict) else str(t.get("priority") or "")).lower() == priority.lower()
        ]

    return {"count": len(tickets), "results": [_normalize_ticket(t) for t in tickets]}
...
def get_ticket(
    ...
    normalized = _normalize_ticket(ticket, comments)

    # Map CRM comments to frontend history format
    history = []
    for c in comments:
        text = c.get("comment") or ""
        is_auto = "[AUTO-REPLY" in text
        ...
        history.append({
            "id": c.get("id"),
            "type": ctype,
            "author": c.get("author_name") or ("Staff" if c.get("is_reply") else "Tenant"),
            "ts": c.get("created"),
            "content": text,
        })

    # Extract latest bot draft if bot is pending
    bot_draft = None
    if normalized.get("bot_pending"):
        auto_comments = [c for c in comments if "[AUTO-REPLY" in (c.get("comment") or "")]
        if auto_comments:
            latest_auto = max(auto_comments, key=lambda c: c.get("created") or "")
            bot_draft = latest_auto.get("comment")
